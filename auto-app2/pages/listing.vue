...
<div class="min-h-screen min-w-screen  bg-gradient-to-br from-red-50 to-primary-500  items-center justify-center  pt-0">
  ...
  <UContainer class="mt-4">
    <UCard :title="title" :ui="{ rounded: 'lg', shadow: 'md' }">
      ...
      <div class="text-center">
        ...
        <UInput size="lg" v-model="searchTerm" placeholder="Search by Name or Model..." :prefix="MagnifyingGlassIcon" icon="i-heroicons-magnifying-glass" class="w-100 justify-center" />
      </UFormGroup>
      </div>
    ...
    <template #content>
      ...
      <div class="grid  grid-cols-2 gap-4 py-5 text-center ">
        ...
        <UFormGroup label="Couleur">
          <USelect v-model="selectedColors" icon="i-heroicons-paint-brush" :items="availableColors" multiple :clearable :placeholder="availableColors.length ? 'By color' : 'No Available Color'" >
        ...
        <UFormGroup label="Modèle">
          <USelect v-model="selectedModels" icon="i-heroicons-swatch" :items="availableModels" multiple :clearable :placeholder="availableModels.length ? 'By model' : 'No Available Model'" />
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div v-for="car in paginatedCars" :key="car.id" class="relative">
          <UCard :ui="{ rounded: 'lg', shadow: 'md' }" class="shadow-lg hover:shadow-xl transition-shadow duration-300">
            <template #header>
      ...
      <div class="flex-1">
        ...
        <p class="text-sm text-gray-500 mt-2">
          ...
          {{ car.color }} • 
          <UIcon size="20" name="i-heroicons-adjustments-horizontal" class="mx-auto align-sub  text-primary-500 " />
        ...
        <UButton  label="View Details"  icon="i-heroicons-eye" class="w-52 justify-center" @click="openModal(car)"/>
        </div>
        
<!-- Modal For Details -->
        <UModal v-model:open="isModalOpen" title='About The Car' description="We've Searched this car far away !" :ui="{ footer: 'justify-end' }" >
          <template #body>
          ...
          <template #footer>
            <div class="justify-center">
              <UButton class="max-w-40 min-w-32 justify-center" label="Great !" color="primary" variant="outline" @click="isModalOpen = false" />
            </div>
          </template>
        </UModal>
      ...
      <div class="justify-center">
        <UPagination v-model:page="currentPage" :total="filteredCars.length" :to="to" :sibling-count="1" show-edges :per-page="itemsPerPage"
        :align="center"
